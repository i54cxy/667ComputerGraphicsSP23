...
struct VertexNormalTexcoord {
    ...
    VertexNormalTexcoord(Point3 v, Vector3 n, Vector2 t) : vertex(v), normal(n), texCoord(t) {};
};
...
public:
    ...
    */
     ...
     */
    void End(const int position_loc, const int normal_loc, const int tex_coord_loc) {
    ...
    */
    void CreateVertexBuffers(const int position_loc, const int normal_loc, const int tex_coord_loc) {
    ...
    /**
    * Creates vertex buffers with texture coordinates.
    */
    void createVertexBuffersWithTexture(const int position_loc, const int normal_loc, const int tex_coord_loc) {
        // Generate vertex buffers for the vertex list and the face list
        glGenBuffers(1, &vbo);
        glGenBuffers(1, &facebuffer);
        ...
        // Bind the vertex list to the vertex buffer object
        glBindBuffer(GL_ARRAY_BUFFER, vbo);
        glBufferData(GL_ARRAY_BUFFER, vnts.size() * sizeof(VertexNormalTexcoord),
            (void*)&vnts[0], GL_STATIC_DRAW);
        ...
        // Bind the vertex buffer, set the vertex position attribute and the vertex normal attribute
        glBindBuffer(GL_ARRAY_BUFFER, vbo);
        glVertexAttribPointer(position_loc, 3, GL_FLOAT, GL_FALSE, sizeof(VertexNormalTexcoord), (void*)0);
        glVertexAttribPointer(normal_loc, 3, GL_FLOAT, GL_FALSE, sizeof(VertexNormalTexcoord), (void*)(sizeof(Point3)));
        glVertexAttribPointer(tex_coord_loc, 2, GL_FLOAT, GL_FALSE, sizeof(VertexNormalTexcoord), (void*)(sizeof(Point3) + sizeof(Vector3)));
        glEnableVertexAttribArray(position_loc);
        glEnableVertexAttribArray(normal_loc);
...
protected:
    ...
    // Vertex normal texture coordinates list
    std::vector<VertexNormalTexcoord> vnts;
